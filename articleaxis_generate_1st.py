...
def generate_core_axis(df, cate_num, category_name):
    ...
    df_article_core.to_csv('articlecore.csv')
...
def main(): 
    ...
    count = 0
    for sentence in sentence_all:
        series = np.zeros(len(keyword_py_lo)+1)
        for w in sentence:
            if w.lower() in keyword_py_lo:
    ...
    Y_original = TSNE(n_components = 2).fit(A)
    Y_original = Y_original.embedding_
    Y_original = np.array([np.array(Y_original[i]) for i in range(len(Y_original))])

    model_k = KMeans(n_clusters = 20, n_jobs = 4, max_iter = 50) #分为k类, 并发数4
    model_k.fit(Y_original)
    ...
    X_reduced0 = copy.copy(Y_original)
    view_move = np.array(view_0)/12 
    X_reduced = movealittle(view_move,X_reduced0,10)


    df_courseaxis_now = pd.DataFrame({'view':view_0,'artid':list(df_detail['ID']),'title':list(df_detail['post_title']),'category':article_cate,'x':X_reduced0[:,0],'y':X_reduced0[:,1]})
    ...
    df_courseaxis_now = df_courseaxis_now[['artid','view','title','category','x','y']]
    df_courseaxis_now.to_csv('articleaxis_aftermove.csv',encoding = 'utf-8')    
    ...
    df_courseaxis_now.to_excel('articleaxis_aftermove.xlsx')
